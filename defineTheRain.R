defineTheRain <- function(x, stdev = 3, tData = NULL)
{
  if (class(tData) == "matrix") {
    ...
    rainResult <- matrix(data = NA, nrow = 2, ncol = 2, 
                         ...
                         byrow = TRUE)
    ch1Low <- calculateMeanSdCluster(x, cluster = c(1), stdev = stdev)
    rainResult[1,1] <- ch1Low[1,1] + ch1Low[2,1]
    ch1High <- calculateMeanSdCluster(x, cluster = c(2), stdev = stdev)
    rainResult[2,1] <- ch1High[1,1] - ch1High[2,1]
    ch2Low <- calculateMeanSdCluster(x, cluster = c(1), stdev = stdev)
    rainResult[1,2] <- ch2Low[1,2] + ch2Low[2,2]
    ch2High <- calculateMeanSdCluster(x, cluster = c(4), stdev = stdev)
